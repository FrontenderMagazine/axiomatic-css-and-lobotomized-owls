...
как вам будет угодно) приучили себя *по умолчанию* стилизировать HTML-элементы. Мы
представляем себе элемент интерфейса, а затем создаем стили для объекта, который
добавляем вручную в разметку.
...
методологию, такой подход не позволяет подключать к работе редакторов контента, как правило, далеких от верстки. Он требует не только знания разметки, отвечающей за
представление, но и доступа к ней, что бы соответствующим образом разметить
контент. WYSIWYG-редакторов и инструментов вроде маркдауна определенно недостаточно
...
за представление, остается вопрос *а надо ли*. Добавление шифров для стилизации в изначально лаконичную разметку непременно приведет к её разрастанию,
...
для него стили, которые уже должны были быть определены выше в каскаде. Добавление
...
Согласно Фрэйну, наоборот — именно количество CSS-селекторов, разрастание кода, может
привести к проблемам; особо он упоминает неиспользуемые, хотя и объявленные CSS-правила.
...
Хорошо, это и гигантские JPEG-изображения и веб-шрифты, которые не были урезаны до необходимого подмножества символов.

Наоборот, одновременный контроль множества элементов с помощью селектора `*` делает
...
представляет ничего, кроме «стилизуем всё подряд». Фокус в том,
...
Теперь мы можем воспользоваться достоинствами [низкой специфичности][11] совиного селектора
...
Обратите внимание, что отступ добавлен только тем параграфам, которые
следуют за другими — это еще одно преимущество селектора по ближайшему соседу.
...
для компактных модулей. В этом примере все элементы, которые требуют отступа, получат
...
виджетам, например, картам, где позиционирование должно быть точным. Это — простой 
способ его отменить. Виджеты такого типа всё чаще и чаще будут реализованы как
веб-компоненты, в которых это правило все равно бы не наследовалось. Это происходит
благодаря [инкапсуляции стилей в теневом DOM][12].

## Красота `em`

Хотя некоторые исключения неизбежны, благодаря использованию единиц `em` для
значений наших отступов они уже автоматически подстраиваются под значения другого свойства:
*font-size*. В каждом случае, когда мы регулируем *font-size*, отступы приспосабливаются
к нему: отступ в одну строку остается отступом в одну строку. Это особенно полезно при
увеличении или уменьшении *font-size* элемента body с помощью `@media`-запросов.

Когда дело касается заголовков, нам выпадает еще большая удача.
Определив кегль шрифта заголовка в `em`, мы сразу получаем соответствующий
заголовку отступ (отступ лида), не написав ни одной дополнительной строки кода.

![Диаграмма][Автоматическое изменение отступа на основе кегля шрифта]

## Строчные элементы

Предполагается, что объявленные стили должны наследоваться. Как CSS, вобще-то, и 
задумывался. Однако я понимаю, как раздражает многих из вас то, насколько
...
если это поможет в дальнейшем, помните, что строчные элементы со стандартным значением
`display: inline` тоже унаследуют верхний отступ, но он не повлияет на их
отображение. Строчные элементы учитывают только горизонтальные отступы, что закреплено
спецификацией и является их стандартным поведением во всех браузерах.
...
с поточным контентом, а поточный контент должен быть основным на странице. Я бы
не советовал сильно полагаться на позиционированный контент
в большинстве интерфейсов, так как он нарушает неявное взаимодействие элементов 
в рамках потока. Даже сетки, с их флоат-колонками, не должны требовать чего-то 
большего, чем простой селектор `.row > *`, применяющий к ним `margin-top: 0`, 
что бы сбросить отступ.
