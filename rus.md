...
виджетам, например, картам, где позиционирование должно быть точным. Это — простой способ его отменить. Все чаще виджеты, вроде этого, исполняются в виде
...
Предполагается, что объявленные стили должны наследоваться. Как CSS, вобще-то,и задумывался. Однако я понимаю как раздражает многих из вас то, насколько
...
с поточным контентом, и он должен быть основным на странице. Я бы не советовал
сильно полагаться на него при использовании для позиционированного контента
в большинстве интерфейсов, так как он нарушает неявное взаимодействие элементов в рамках потока (?). Даже сетки, с их флоат-колонками не должны требовать чего то большего, чем простой селектор `.row > *`, применяющий к ним `margin-top: 0`, что бы сбросить отступ.
